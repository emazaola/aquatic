...
```{r sel_bands}
sel_bands <- sentinel2_band_mapping$planetary_computer_v1[c("B02", "B03", "B04", "B05", "B06", "B08", "B8A","B11")]
```
...
# Hay length(sel_bands) bandas por archivo .


num_bandas_por_archivo <- length(sel_bands) # Confirma este número (B, G, R, N, S1)
```
...
nombres_capas <- paste(df_metadatos$banda_nombre,
                       df_metadatos$fecha,
                       sep = "_")
...
dir.create("../../process/geoprocess/s2_stacks/", recursive = TRUE, showWarnings = FALSE)
...
for (nombre in names(S2_serie)) {
  archivo <- file.path("../../process/geoprocess/s2_stacks/",
                       paste0(nombre, ".tif"))
...
## Visualización RGB
```{r}
names(S2_serie$`S2_2025-05-22`)
```

Para RGB, las bandas roja, verde y azul deben coincidir con sus canales.
...
calcular_indices_aqua <- function(stack) {
  ...
  for (fecha in fechas) {
    ...
    N2 <- stack[[paste0("N2_", fecha)]]
    RE1 <- stack[[paste0("RE1_", fecha)]]
    RE2 <- stack[[paste0("RE2_", fecha)]]
    S1 <- stack[[paste0("S1_", fecha)]]
    
    # MDWI
#     mdwi <- (G - S1) / (G + S1)
#     names(mdwi) <- paste0("MDWI_", fecha)
#     MDWI_list[[fecha]] <- mdwi
    
    # NDCI
    ndci <- (RE1 - R) / (RE1 +  R )
    names(ndci) <- paste0("NDCI_", fecha)
    NDCI_list[[fecha]] <- ndci
    
    # FAI
    
    # Simplified FAI (approx): 
      # following Hu (2009) approach — baseline interpolation for NIR
      # baseline_NIR = B4 + (B11 - B4) * (lambda_NIR - lambda_RED) / (lambda_SWIR - lambda_RED)
      # For simplicity use B8A (865 nm) as NIR measurement
    
#    lambda_nir <- 665
#    lambda_red <- 865
#    lambda_swir <- 1610
    
#    baseline_nir <- R + (S1 - R) * ((lambda_nir - lambda_red)/(lambda_swir - lambda_red))
    
#    fai <- N2 - baseline_nir
#    names(fai) <- paste0("FAI_", fecha)
#    FAI_list[[fecha]] <- fai
    
    
    
    # MCI (simplified) -> height at B5 over baseline between B4 and B6
    
#     baseline_b5 <- R + (RE2 - R * ((705 - 665) / (740 - 665)))
                         
#     mci <- RE1 - baseline_b5
#     names(mci) <- paste0("MCI_", fecha)
#     MCI_list[[fecha]] <- mci
...
#     ndti <- (G - N) / (G + N)
#     names(ndti) <- paste0("NDTI_", fecha)
#     NDTI_list[[fecha]] <- ndti
     
    # NDAVI (example for aquatic vegetation): (NIR - Blue)/(NIR + Blue)
#    ndavi <- (N2 - B / N2 + B)
#    names(ndavi) <- paste0("NDAVI_", fecha)
#    NDAVI_list[[fecha]] <- ndavi
  
   
  }
  
  # Convertir listas a SpatRaster con nombres adecuados
#   MDWI <- rast(MDWI_list); names(MDWI) <- nombres_mdwi
  NDCI <- rast(NDCI_list); names(NDCI) <- nombres_ndci
#  FAI <- rast(FAI_list); names(FAI) <- nombres_fai
#  MCI <- rast(MCI_list); names(MCI) <- nombres_mci  
#  NDTI <- rast(NDTI_list); names(NDTI) <- nombres_ndti
#  NDAVI <- rast(NDAVI_list); names(NDAVI) <- nombres_ndavi  
...
```{r}
# Calcular índices
indices_aqua <- calcular_indices_aqua(s2_stack)
```


```{r index_in_name}
# names(indices_aqua$MDWI) <- paste0("MDWI_", names(indices_aqua$MDWI))
 names(indices_aqua$NDCI)  <- paste0("NDCI_",  names(indices_aqua$NDCI))
# names(indices_aqua$FAI)   <- paste0("FAI_",   names(indices_aqua$FAI))
# names(indices_aqua$MCI) <- paste0("MCI_", names(indices_aqua$MCI))
# names(indices_aqua$NDTI)   <- paste0("NDTI_",   names(indices_aqua$NDTI))
# names(indices_aqua$NDAVI) <- paste0("NDAVI_", names(indices_aqua$NDAVI))
...
calcular_indices_aqua <- function(stack) {
  ...
  # Convertir listas a SpatRaster con nombres adecuados
  MDWI <- rast(MDWI_list)
  names(MDWI) <- nombres_mdwi
  
  NDCI <- rast(NDCI_list)
  names(NDCI) <- nombres_ndci
  
  FAI <- rast(FAI_list)
  names(FAI) <- nombres_fai
  
  MCI <- rast(MCI_list)
  names(MCI) <- nombres_mci  
  
  NDTI <- rast(NDTI_list)
  names(NDTI) <- nombres_ndti
  
  NDAVI <- rast(NDAVI_list)
  names(NDAVI) <- nombres_ndavi  
...
```{r}
names(indices_aqua$FAI)
```


```{r}
# Función: Guarda capas raster en archivos GeoTIFF
guardar_indices <- function(lista_indices, carpeta_salida = "../../process/geoprocess/index_AQUA/") {
...
```{r}
# Guardarlos en disco
guardar_indices(indices_aqua, 
                carpeta_salida = "../../process/geoprocess/index_AQUA")
...
guardar_stacks_completos <- function(lista_indices, carpeta_salida = "../../process/geoprocess/index_AQUA/") {
...
guardar_stacks_completos(indices_aqua, carpeta_salida = "../../process/geoprocess/index_AQUA/")
...
### Imágenes Índices acuáticos

```{r}
# Paleta y cortes comunes
colores_aqua <- c("red", "orange", "yellow", "lightgreen", "forestgreen")
cortes_aqua <- c(-Inf, 0, 0.1, 0.2, 0.4, Inf)
...
```{r}
# Ruta base de salida
carpeta_base <- "../../process/figures/index_AQUA/"

# Iterar sobre cada índice de vegetación
for (nombre_indice in names(indices_aqua)) {
  
  stack_indice <- indices_aqua[[nombre_indice]]
...
### Imágenes Índices de vegetación
